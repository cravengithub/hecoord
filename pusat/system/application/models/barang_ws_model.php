...
/**
 * Description of barang_ws_model
 *
 * @author craven
 */
class Barang_ws_model extends Model {
    //put your code here
    function  __construct() {
        parent::Model();
    }
    function get_barang_ws() {
    ...
    function get_barang_ws_by_criteria($data) {
        $this->db->from('jatah_barang_cabang jc');
        $this->db->join('barang b','b.kode_barang=jc.kode_barang');
        $this->db->join('kantor k','k.id_kantor=jc.id_kantor');
    ...
    function get_barang_ws_by_id($id) {
         $this->db->join('barang b','b.kode_barang=jc.kode_barang');
        $this->db->join('kantor k','k.id_kantor=jc.id_kantor');
    ...
    function get_barang_ws_all() {
         $this->db->join('barang b','b.kode_barang=jc.kode_barang');
        $this->db->join('kantor k','k.id_kantor=jc.id_kantor');
        $res = $this->db->get('jatah_barang_cabang jc');
        if($res->num_rows()>0) {
            return $res;
        }
        return FALSE;
